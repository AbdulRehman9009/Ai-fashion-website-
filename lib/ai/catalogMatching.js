...
export function normalizeProductAudience(value) {
  const text = String(value || "").trim().toLowerCase();
  if (["men", "mens", "male", "man", "boys", "boy", "gents"].includes(text)) return "male";
  if (["women", "womens", "female", "woman", "girls", "girl", "ladies"].includes(text)) return "female";
  if (["unisex", "any", "all", "neutral"].includes(text)) return "any";
  return null;
}
...
export function inferProductGender(product) {
  const explicitAudience = normalizeProductAudience(product.audience || product.gender || product.attributes?.gender);
...
export function genderMatches(productGender, preferredGender) {
  if (!preferredGender || preferredGender === "any" || preferredGender === "unknown") return true;
  if (!productGender || productGender === "unknown") return false; // Strictly require explicit match if user specifies gender
  if (productGender === "any") return true; // Unisex fits all
...
export function scoreProduct(product, terms, preferredGender) {
  ...
  let score = productGender === preferredGender ? 10 : (productGender === "unknown" ? 1 : 4);
