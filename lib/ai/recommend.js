...
export async function generateOutfitRecommendations(imageBuffer, mimeType, context, userId) {
  try {
    await connectDB();
    const model = apiKey ? genAI.getGenerativeModel({ model: GEMINI_MODEL }) : null;
    ...
    const recommendationPrompt = `
      ...
      Select the top 3 outfits only from the listed product IDs.
      The product list has already been filtered by outfit gender fit (${genderPreference}); do not invent IDs and do not recommend another gender category.
      For each, provide a styling tip and why it suits the user/event.
