 ...
 * Fixed: uses correct Order schema field paths.
 */
export async function createEarningsForOrder(order) {
    try {
        const config = await getPaymentDistributionConfig();
        const distribution = calculatePaymentDistribution(
            order.pricing?.grandTotal || 0,  // FIXED: was order.grandTotal
            order.urgent || false,            // FIXED: was order.isUrgent
            config
        );
        ...
        if (distribution.tailorAmount > 0 && order.assignedTailor) {
            earnings.push({
                user: order.assignedTailor._id || order.assignedTailor,
                order: order._id,
        ...
        // Delivery earning — FIXED: use order.delivery (Delivery doc ref), then look up assignedTo
        if (distribution.deliveryAmount > 0 && order.delivery) {
            let deliveryUserId = null;
            // If delivery is populated, use assignedTo directly
            if (order.delivery.assignedTo) {
                deliveryUserId = order.delivery.assignedTo;
            } else {
                // If delivery is just an ObjectId, look it up
                const deliveryDoc = await Delivery.findById(order.delivery).select("assignedTo").lean();
                deliveryUserId = deliveryDoc?.assignedTo;
            }
            if (deliveryUserId) {
                earnings.push({
                    user: deliveryUserId,
        ...
        // Platform fee — FIXED: find admin user instead of null (user is required)
...
/**
 * Process a payout batch for a specific user.
 *
 * @param {string} userId - The user to pay out
 * @param {string[]} earningIds - Array of earning IDs to process
 */
export async function processPayoutBatch(userId, earningIds) {
    try {
