...
const TEMPLATES = {
    verification: (token, name) => ({
        subject: "Verify your email - Style Genie",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #ea580c;">Welcome to Style Genie!</h1>
        <p>Hi ${name || "there"},</p>
        ...
        <p style="color: #999; font-size: 12px;">Style Genie - AI-Powered Fashion Platform</p>
      </div>
    `
    }),

    passwordReset: (token, name) => ({
        subject: "Reset your password - Style Genie",
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    orderConfirmation: (order) => ({
        subject: `Order Confirmed #${order._id.toString().slice(-6)} - Style Genie`,
        html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
async function sendWithResend(to, subject, html) {
    ...
    const result = await resend.emails.send({
        from: process.env.EMAIL_FROM || 'Style Genie <noreply@stylegenie.com>',
        to,
        subject,
        html
    });
