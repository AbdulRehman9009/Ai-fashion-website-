...
const VALID_SOCIAL_ROLES = ["USER", "TAILOR", "DELIVERY", "SHOPKEEPER"];

async function getPendingOAuthRole() {
  try {
    const cookieStore = await cookies();
    const requestedRole = cookieStore.get("style_genie_oauth_role")?.value?.toUpperCase();
    return VALID_SOCIAL_ROLES.includes(requestedRole) ? requestedRole : "USER";
  } catch {
    return "USER";
  }
}
  ...
  callbacks: {
    async signIn({ user, account, profile }) {
      if (account.provider === "google" || account.provider === "github") {
        try {
          ...
          const requestedRole = await getPendingOAuthRole();
    ...
    async jwt({ token, user, trigger, session }) {
      if (user) {
        ...
        // Check profile completion only at sign-in (not on every request)
        try {
          await connectDB();
          ...
          console.error("Error checking profile status at sign-in:", error);
