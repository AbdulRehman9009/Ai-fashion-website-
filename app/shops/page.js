            ...
            <div className="container mx-auto px-4 py-8">
                <div className="mb-8 max-w-md mx-auto sm:mx-0">
                    <div className="relative group">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-hover:text-orange-500 transition-colors" />
                        <Input
                            placeholder="Search shops by name or location..."
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="pl-10 h-10 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-orange-500/20 transition-all shadow-sm"
                        />
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
                        ...
                        <h3 className="text-lg font-semibold text-slate-900 dark:text-white">No shops found</h3>
                    ...
                    <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {filteredShops.map((shop) => (
                            <Link key={shop._id} href={`/shops/${shop._id}`}>
                                <Card className="overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full group bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
                                    ...
                                    <CardContent className="p-5 relative pt-12">
                                        <div className="absolute -top-10 left-5">
                                            {shop.logo ? (
                                                <img src={shop.logo} alt={shop.name} className="w-20 h-20 rounded-xl object-cover border-4 border-white dark:border-slate-900 shadow-lg bg-white dark:bg-slate-800" />
                                            ) : (
                                                <div className="w-20 h-20 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center border-4 border-white dark:border-slate-900 shadow-lg">
                                                    <Store className="h-10 w-10 text-slate-400 dark:text-slate-500" />
                                                </div>
                                            )}
                                        </div>
                                        <h3 className="font-bold text-lg text-slate-900 dark:text-white mb-1 line-clamp-1 group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
                                            {shop.name}
                                        </h3>
                                        {shop.description && (
                                            <p className="text-sm text-slate-600 dark:text-slate-400 mb-3 line-clamp-2 leading-relaxed">{shop.description}</p>
                                        )}
                                        ...
                                        {shop.categoryPermissions?.length > 0 && (
                                            <div className="flex flex-wrap gap-1.5 mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                                                {shop.categoryPermissions.slice(0, 3).map((category, idx) => (
                                                    <Badge key={idx} variant="secondary" className="text-[10px] px-1.5 h-5 font-normal bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">
                                                        {category}
                                                    </Badge>
                                                ))}
                                            </div>
